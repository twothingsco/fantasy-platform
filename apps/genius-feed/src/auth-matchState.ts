import { geniusMatchState } from './config.ts';
// --- 1. TYPE DEFINITIONS ---
...
const AUTH_URL = 'https://auth.api.geniussports.com/oauth2/token';

const authData = {
  client_id: geniusMatchState.clientId,
  client_secret: geniusMatchState.clientSecret,
...
async function fetchNewToken(): Promise<string> {
  ...
  const formBody = new URLSearchParams(authData);
...
export async function getAPIData (url: string){ 
    ...
    headers: {
        ...
        'x-api-key': geniusMatchState.apiKey,
