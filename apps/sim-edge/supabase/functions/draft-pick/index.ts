...
Deno.serve(async (req: Request) => {
    // console.log('[RunDraft] Received request:', req);
    ...
    try {
        ...
        console.log(`[Pick Player] Received request to Pick Player for draft: Season ${season}`);
    ...
    //    EdgeRuntime.waitUntil(draftExecutionPromise); // Keep function alive until draft completes
        ...
        return new Response(JSON.stringify({
            ...
            function_id: Deno.env.get('FUNCTION_NAME') || 'draft-pick', // Example to return function ID
        }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
