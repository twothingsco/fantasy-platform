...
import { getLeagueTeams } from './YahooService.ts';
...
Deno.serve(async (req: Request) => {
    ...
    try {
        const { season, currentWeek =0 , leagueId = 1, access_token, refresh_token, mode_id } = await req.json();
        ...
        console.log(`Received request to generate YAHOO teams: Season ${season}, League ${leagueId}`);
        // await broadcastTeamUpdate('status', {
        //     message: `Initalizing....`,
        //     progress: 0
        // });

        const generationPromise =  getLeagueTeams(leagueId, season, currentWeek, access_token, refresh_token, mode_id); // Call the function to generate teams
    ...
    //    const results = await generationPromise
       generationPromise
        ...
        return new Response(JSON.stringify({
            ...
            message: 'YAHOO Team generation process initiated. Updates will be broadcast via Realtime.',
        }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 202,
        });
        ...
        console.error('Error in create-yahoo Edge Function (outer catch):', error);
        await broadcastTeamUpdate('error', {
            message: 'YAHOO Team generation initiation failed!', // Changed message to differentiate from background task failure
            error: error.message
        });
