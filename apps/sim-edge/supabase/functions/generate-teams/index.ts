...
import { generateLeagueTeams } from './LeagueSetupGraph.ts';
...
Deno.serve(async (req: Request) => {
    ...
    try {
        const { season, teamCount, league_name, auth_user_id, mode_id } = await req.json();

        if (typeof season !== 'number' || typeof teamCount !== 'number') {
        ...
        console.log(`Received request to generate teams: Season ${season}, Teams ${teamCount}`);
        // await broadcastTeamUpdate('status', {
        //     message: `Initalizing....`,
        //     progress: 0
        // });

        const generationPromise =  generateLeagueTeams(season, teamCount, league_name, auth_user_id, mode_id);
    ...
    //    const results = await generationPromise
       generationPromise
        ...
        return new Response(JSON.stringify({
            ...
            message: 'Team generation process initiated. Updates will be broadcast via Realtime.',
        }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 202,
        });
        ...
        console.error('Error in generate-teams Edge Function (outer catch):', error);
        await broadcastTeamUpdate('error', {
            message: 'Team generation initiation failed!', // Changed message to differentiate from background task failure
            error: error.message
        });
