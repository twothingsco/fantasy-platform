import {UserRosterInsert, GameWeekTeamPlayerInsert,PlayerData} from "./types"; // Import types from types.ts
import { error } from 'console';
import { supabase } from './supabaseClient';  // Import the Supabase client


export async function getAllPlayersforSeason(seasonYear: number): Promise<Array<PlayerData>> {
    console.log(`Fetching all players for season ${seasonYear}...`);
    const batchSize = 500;
  ...
  try {
    while (hasMore) {
      const { data: currentPlayersBatch, error: fetchBatchError } = await supabase
        .from('nfl_players')
        .select('id as player_id, name as player_name, nfl_id, position_type, 0 as game_week_id, "" as scoring_mode, 0 as fantasy_score')
        .range(offset, offset + batchSize - 1); // range is inclusive
      ...
      if (currentPlayersBatch && currentPlayersBatch.length > 0) {
        allPlayers = allPlayers.concat(currentPlayersBatch);
        offset += batchSize;
        if (currentPlayersBatch.length < batchSize) {
          hasMore = false; // Last batch was smaller than batchSize, so no more data
        }
      } else {
